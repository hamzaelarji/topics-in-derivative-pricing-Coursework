...
![png](02_nelson_siegel_files/02_nelson_siegel_13_0.png)
...
![png](02_nelson_siegel_files/02_nelson_siegel_20_0.png)
...
![png](02_nelson_siegel_files/02_nelson_siegel_23_0.png)
...
```
    ...
    currency model  mean_bp  median_bp  p95_bp  max_bp
         ...
         USD    NS     6.28       5.70   10.80   21.77
         USD   eNS     5.21       5.07    7.72   13.42
         USD   NSS     1.32       0.93    3.73    7.61
...
**Critical JPY signal:** the NS→eNS gap is 6.5→2.6 bp, i.e. a 60% reduction. This is by far the largest gap among all currencies. This means that the global λ (0.5) is very poorly suited to JPY — the optimal λ by date for JPY must be very different from 0.5. For the report, this is a strong argument: *“the pooled λ penalizes JPY, which the VAE avoids because it does not impose a parametric form.”*

**AUD signal:** 10 bp in NS is the highest. The AUD curve likely has a more complex or more volatile shape over the period. Worth mentioning.


* The NS→NSS gap is huge (6.77→1.52 bp, ×4.5). NSS with 4 parameters fits almost perfectly (this is consistent with the literature).
* The p95 of NS is 13.10 bp -> the tails are heavy, meaning that on some days NS really fails. These days likely correspond to the most complex curves (strongly inverted, or with a double hump).

* the key takeaway is *“NS 3D ≈ 6.8 bp pooled → the 2D VAE should aim for this level or better to validate Sokol’s thesis.”*
...
![png](02_nelson_siegel_files/02_nelson_siegel_32_0.png)
...
![png](02_nelson_siegel_files/02_nelson_siegel_34_0.png)
...
![png](02_nelson_siegel_files/02_nelson_siegel_36_0.png)
...
**β₁ (level):** All currencies rise gradually from 2023 to 2026, consistent with the rate-hiking cycle followed by a plateau. JPY starts very low (~1.5%) and catches up with the others — consistent with BoJ normalization. Correct.

**β₂ (slope):** Starts positive (normally upward-sloping curves) then turns negative around late 2023/early 2024 for most currencies (inversion). JPY stays close to zero throughout — this is exactly why its β₂–slope correlation is almost zero (-0.09): its slope did not vary like the others. This is the direct explanation for the JPY anomaly in the correlation table.

**β₃ (curvature):** A lot of noise, very small values (±0.05), and JPY shows a visible spike in early 2023. This is expected — β₃ is the residual factor and the least stable.
...
![png](02_nelson_siegel_files/02_nelson_siegel_41_0.png)
...
![png](02_nelson_siegel_files/02_nelson_siegel_44_0.png)
...
The heatmap confirms the predicted structural pattern: NS errors are systematically larger at the 30Y tenor for all currencies. This is consistent with theory, NS is an exponential form that flattens toward infinity, and the long end can diverge from real data.

AUD 30Y = 17.6 bp, the worst residual in the entire heatmap. This explains why AUD has the highest overall RMSE (10 bp). The AUD curve has a 30Y point that does not align with the NS shape.

JPY 10Y = 9.2 bp, unusual. The JPY 10Y seems to be systematically poorly captured by NS, while other JPY tenors are fine. This suggests the JPY curve has a “hump” shape precisely around the 10Y (linked to the BoJ’s YCC policy on the 10Y, which has distorted that maturity).

GBP, the residuals are the lowest among all currencies across almost all tenors. This confirms that GBP is the most regular and NS-compatible currency.

15Y is consistently the most problematic tenor for AUD, DKK, EUR, and USD, it corresponds to the intermediate maturity of the hump that NS struggles to capture with a single β₃ term.
